...
<script>
import { resolveNavLinkItem, isExternal, ensureExt } from '../util/index'
...
export default {
  ...
  computed: {
    currentPage() {
      const firstClassLinks = this.userLinks.filter(n => !n.items || !n.items.length)
      const currentLink = firstClassLinks.find(l => this.currentPath(this.$page.path).startsWith(l.link))
      return currentLink ? [currentLink.link] : [this.currentPath(this.$page.path)]
    },
